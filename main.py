...
from model import ConvE, ConvR, DistMult, Complex
...
def preprocess(dataset_name, delete_data=False):
    ...
    p = Pipeline(args.data, delete_data, keys=input_keys,
                 skip_transformation=True)
    p.add_sent_processor(ToLower())
    p.add_sent_processor(CustomTokenizer(lambda x: x.split(' ')), keys=[
                         'e2_multi1', 'e2_multi2'])
    ...
    for path, name in zip([train_path, dev_ranking_path, test_ranking_path], ['train', 'dev_ranking', 'test_ranking']):
        ...
        p.add_sent_processor(CustomTokenizer(lambda x: x.split(' ')), keys=[
                             'e2_multi1', 'e2_multi2'])
        p.add_post_processor(ConvertTokenToIdx(keys2keys=keys2keys), keys=[
                             'e1', 'rel', 'rel_eval', 'e2', 'e2_multi1', 'e2_multi2'])
        p.add_post_processor(StreamToHDF5(
            name, samples_per_file=1000, keys=input_keys))
        p.execute(d)
...
def main(args, model_path):
    ...
    train_batcher = StreamBatcher(args.data, 'train', args.batch_size,
                                  randomize=True, keys=input_keys, loader_threads=args.loader_threads)
    dev_rank_batcher = StreamBatcher(args.data, 'dev_ranking', args.test_batch_size,
                                     randomize=False, loader_threads=args.loader_threads, keys=input_keys)
    test_rank_batcher = StreamBatcher(args.data, 'test_ranking', args.test_batch_size,
                                      randomize=False, loader_threads=args.loader_threads, keys=input_keys)
    ...
    if args.model is None:
        ...
        print(model)
    elif args.model == 'convr':
        model = ConvR(args, vocab['e1'].num_token, vocab['rel'].num_token)
        print(model)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for i, str2var in enumerate(train_batcher):
            ...
            # label smoothing
            e2_multi = ((1.0-args.label_smoothing)*e2_multi) + \
                (1.0/e2_multi.size(1))
        ...
        with torch.no_grad():
            if epoch % 5 == 0 and epoch > 0:
                ranking_and_hits(model, dev_rank_batcher,
                                 vocab, 'dev_evaluation')
            if epoch % 5 == 0:
                if epoch > 0:
                    ranking_and_hits(model, test_rank_batcher,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', type=int, default=17,
                        metavar='S', help='random seed (default: 17)')
    ...
    parser.add_argument('--model', type=str, default='conve',
                        help='Choose from: {conve, distmult, complex}')
    ...
    model_name = '{2}_{0}_{1}'.format(
        args.input_drop, args.hidden_drop, args.model)
